...
import Modify_2_employee_information from "@/components/modals/forms/modify/Modify_2_employee_information ";

export default function Employee_information() {
  const {
    employeeIsLoading,
    employeeIsError,
    employeError,
  ...
  const employee = uselistOfEmployeesStore((state) => state.listOfEmployees);
  const [filterred, setfiltered] = useState([]);
  const [items, setItem] = useState([]);
  const cvss = useRef();

  useEffect(() => {
    setfiltered(employee);
    setItem(employee);
  ...
  const handleSubmit =async  (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:3000/csv/import-csv",
        formData,
        {
          headers: {
...
<form onSubmit={handleSubmit} className="w-1/2 h-full px-2 rounded-md text-sm flex justify-center items-center gap-2 border-2 text-nowrap font-semibold border-[#009BFF] text-[#009BFF] text-center relative">
      <input
        type="file"
        accept=".csv"
        name="csvFile"
                  ...
                  <TableRow key={employee.crew_id + employee + index}>
                    ...
                    <TableCell>
                      <span className="flex space-x-2 ">
                        {employee?.availability?.sunday.length > 0 && (
                          <span
                            className={`text-sm flex justify-center nowrap items-center h-8 w-8 rounded-lg font-bold   ${
                              employee.status === "Inactive"
                                ? "text-[#565656] bg-[#AFAFAF] "
                                : "   bg-[#C0DFFD] text-[#56A7F4]"
                            }  `}
                          >
                            S
                        ...
                        {employee?.availability?.monday.length > 0 && (
                          <span
                            className={`text-sm flex justify-center nowrap items-center h-8 w-8 rounded-lg font-bold
                            ${
                              employee.status === "Inactive"
                        ...
                        {employee?.availability?.tuesday.length > 0 && (
                          <span
                            className={`text-sm flex justify-center nowrap items-center h-8 w-8 rounded-lg font-bold ${
                              employee.status === "Inactive"
                                ? "text-[#565656] bg-[#AFAFAF] "
                        ...
                        {employee?.availability?.wednesday.length > 0 && (
                          <span
                            className={`text-sm flex justify-center nowrap items-center h-8 w-8 rounded-lg font-bold ${
                              employee.status === "Inactive"
                                ? "text-[#565656] bg-[#AFAFAF] "
                        ...
                        {employee?.availability?.thursday.length > 0 && (
                          <span
                            className={`text-sm flex justify-center nowrap items-center h-8 w-8 rounded-lg font-bold ${
                              employee.status === "Inactive"
                                ? "text-[#565656] bg-[#AFAFAF] "
                        ...
                        {employee?.availability?.friday.length > 0 && (
                          <span
                            className={`text-sm flex justify-center nowrap items-center h-8 w-8 rounded-lg font-bold ${
                              employee.status === "Inactive"
                                ? "text-[#565656] bg-[#AFAFAF] "
                        ...
                        {employee?.availability?.saturday.length > 0 && (
                          <span
                            className={`text-sm flex justify-center nowrap items-center h-8 w-8 rounded-lg font-bold ${
                              employee.status === "Inactive"
                                ? "text-[#565656] bg-[#AFAFAF] "
                    ...
                    <TableCell>
                      ...
                      >
                        ...
                        <DialogContent className="lg:w-[80%] ">
                          ...
                          <DialogHeader className=" min-h-[80vh]   max-h-[80vh]    pb-16  bg-white text-center    ">
                            <DialogTitle className="text-center font-bold flex w-full justify-center">
                              MODIFY CREW INFORMATION
                            </DialogTitle>
                            <Modify_2_employee_information
                          ...
                          const statusfun = async () => {
                            ...
                            try {
                              const update = await axios.put(
                                `http://localhost:3000/employee/status/${employee.crew_id}`,
                                {
