...
setEmployeeOnLeave} = useGenerateScheduleStore()
  ...
  const submit = (e) => {
    ...
    if(isProcessing){
      ...
      try {
        ...
        // aftera success of generated scheudle
        setIsProcessing(true)
          ...
          const generate = async () => {
            ...
            const getgenerate = await axios.get("http://localhost:3000/sched/schedule",{
              headers: {
...
console.log("helllo",getgenerate)
            

              setGeneratedSchedule(
                // getgenerate.data.schedule.slice(0,100)

                {
                  production:{
                  monday:getgenerate.data.schedule.filter(filt => filt.area_name === "Production" && filt.day_id == 1),
                  tuesday:getgenerate.data.schedule.filter(filt => filt.area_name === "Production" && filt.day_id == 2),
                  wednesday:getgenerate.data.schedule.filter(filt => filt.area_name === "Production" && filt.day_id == 3),
                  thursday:getgenerate.data.schedule.filter(filt => filt.area_name === "Production" && filt.day_id == 4),
                  friday:getgenerate.data.schedule.filter(filt => filt.area_name === "Production" && filt.day_id == 5),
                  saturday:getgenerate.data.schedule.filter(filt => filt.area_name === "Production" && filt.day_id == 6),
                  sunday:getgenerate.data.schedule.filter(filt => filt.area_name === "Production" && filt.day_id == 7),
},
                  service:{
                  monday:getgenerate.data.schedule.filter(filt => filt.area_name === "Service" && filt.day_id == 1),
                  tuesday:getgenerate.data.schedule.filter(filt => filt.area_name === "Service" && filt.day_id == 2),
                  wednesday:getgenerate.data.schedule.filter(filt => filt.area_name === "Service" && filt.day_id == 3),
                  thursday:getgenerate.data.schedule.filter(filt => filt.area_name === "Service" && filt.day_id == 4),
                  friday:getgenerate.data.schedule.filter(filt => filt.area_name === "Service" && filt.day_id == 5),
                  saturday:getgenerate.data.schedule.filter(filt => filt.area_name === "Service" && filt.day_id == 6),
                  sunday:getgenerate.data.schedule.filter(filt => filt.area_name === "Service" && filt.day_id == 7),
}
                }
              )
            ...
            setIsDone(true);
            navigate("/shift_manager/generate_schedule/step_3");
  ...
  // },[isDone])
  return (
    <div className={`w-full rounded-lg p-4 text-sm ${isProcessing ? " bg-transparent " : " bg-white "}`}>
...
Please Check Employee on Leave before generating schedule
        </p>
        <div className="flex justify-center ">
          <button
            type="submit"
            className={`bg-primary text-white  rounded-lg p-2 mt-4 ${
              isProcessing ? "bg-red-500" : "bg-green-500"
            }`}
          >
            {isProcessing ? "Cancel" : "Generate Schedule"}
          </button>
        </div>
      </form>
    </div>
